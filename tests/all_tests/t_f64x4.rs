...
#[test]
fn impl_neg_for_f64x4() {
  let a = f64x4::from([1.0, -2.0, 0.0, -0.0]);
  let expected = f64x4::from([-1.0, 2.0, -0.0, 0.0]);
  assert_eq!(-a, expected);

  // Verify that 0.0 and -0.0 are properly sign-flipped
  let zero = f64x4::splat(0.0);
  let neg_zero = -zero;
  let bits: [u64; 4] = cast(neg_zero);
  assert_eq!(bits, [0x8000000000000000u64; 4]); // All should be -0.0

  let neg_zero_input = f64x4::splat(-0.0);
  let pos_zero = -neg_zero_input;
  let bits: [u64; 4] = cast(pos_zero);
  assert_eq!(bits, [0x0000000000000000u64; 4]); // All should be 0.0
}
...
fn impl_f64x4_recip() {
  ...
  {
    let expected = f64x4::splat(value.recip());
    let actual = f64x4::splat(value).recip();
    // Use bitwise equality to accept NaNs as equal.
    assert_eq!(expected ^ actual, f64x4::ZERO);
  }
}
...
#[test]
fn impl_f64x4_recip_sqrt() {
  for value in [0.0, 1.0, 3.6, 34579.2, f64::NAN, f64::INFINITY]
    .into_iter()
    .flat_map(|value| [value, -value])
  {
    let expected = f64x4::splat(value.sqrt().recip());
    let actual = f64x4::splat(value).recip_sqrt();
    // Use bitwise equality to accept NaNs as equal.
    assert_eq!(expected ^ actual, f64x4::ZERO);
  }
}
...
fn impl_f64x4_exp2() {
  for x in
    [-2.0, -1.1, 0.0, 1.3, 1.5, 2.0, 10.4, 100.5, 1000.2, 1023.0, -1074.0]
  {
    let _: f64 = x;
    ...
    assert!(diff_from_std[0] <= (expected.to_array()[0] * 1e-12).max(5e-324));
  }
